...
public class Run {
	public static void main(String[] args) throws IOException, AndrolibException, InterruptedException {
		try {
	...
	private static void cmdDecode(String[] args) throws InvalidArgsError, AndrolibException {
		...
		if (args.length == i + 2) {
			...
			outName = args[i];
			outName = outName.endsWith(".apk") ? outName.substring(0, outName.length() - 4) : outName + ".out";
		...
		try {
			...
			System.out.println("Destination directory (" + outDir.getAbsolutePath() + ") "
					+ "already exists. Use -f switch if you want to overwrite it.");
			System.exit(1);
		} catch (CantFindFrameworkResException ex) {
			System.out.println("Can't find framework resources for package of id: " + String.valueOf(ex.getPkgId())
					+ ". You must install proper " + "framework files, see project website for more info.");
			System.exit(1);
		}
	}

	private static void cmdBuild(String[] args) throws InvalidArgsError, AndrolibException {
		...
		new Androlib().build(new File(appDirName), outFile, forceBuildAll, debug);
	}
	...
	private static void cmdPublicizeResources(String[] args) throws InvalidArgsError, AndrolibException {
		if (args.length != 1) {
			throw new InvalidArgsError();
		}
